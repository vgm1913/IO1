...
-- Description:	Get all associated Portfolios to the Account Portfolio ID passed
...
-- =============================================
CREATE PROCEDURE CS_GetAccountAssociations 
	@PORT_ID bigint = 0
...
BEGIN
	...
	DECLARE @ACCT_TYPE_ID int
	
	SET @ACCT_TYPE_ID = (SELECT PortfolioTypeUid FROM PortfolioType WHERE PortfolioTypeAbbr = 'ACCT')
	IF (@PORT_ID = 0)	-- user passed nothing
	BEGIN
		PRINT 'usage: CG_GetGroupAssociations @PORT_ID bigint, [, @AS_OF_DATE date ] [, @A_PORT_TYPE int] [, @JSON int ]'
		GOTO Branch_EXIT
	END
	...
	IF (@A_PORT_TYPE = 0)
		...
		SELECT PortfolioTypeUid FROM PortfolioType WHERE PortfolioTypeUid != @ACCT_TYPE_ID
	ELSE
		INSERT INTO #myPortTypes
		SELECT PortfolioTypeUid FROM PortfolioType WHERE PortfolioTypeUid = @A_PORT_TYPE AND PortfolioTypeUid != @ACCT_TYPE_ID

	IF (SELECT COUNT(*) FROM #myPortTypes) = 0
	BEGIN
		PRINT 'usage: CS_GetAccountAssociations @PORT_ID bigint, @AS_OF_DATE date [, @A_PORT_TYPE int] [,	@JSON int ]'
	...
	BEGIN
		...
		AND a.PortfolioAUid = e.AcctPortfolioUid
